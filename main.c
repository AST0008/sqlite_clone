...
const uint32_t LEAF_NODE_RIGHT_SPLIT_COUNT = LEAF_NODE_MAX_CELLS - LEAF_NODE_LEFT_SPLIT_COUNT;
    ...
    */
    for (int32_t i = LEAF_NODE_MAX_CELLS - 1; i >= 0; i--)
    {
        ...
        *leaf_node_num_cells(new_node) += LEAF_NODE_RIGHT_SPLIT_COUNT;
        ...
        // uint32_t key = *leaf_node_key(old_node, i);
        // uint32_t value = *leaf_node_value(old_node, i);
        // leaf_node_insert(destination_node, key, value);
    }
}
...
{
    return internal_node_cell(node, key_num) + INTERNAL_NODE_KEY_SIZE;
}
