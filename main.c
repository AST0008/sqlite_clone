...
// Replace all the const uint32_t offset calculations with #defines
...
#define LEAF_NODE_RIGHT_SPLIT_COUNT (LEAF_NODE_MAX_CELLS - LEAF_NODE_LEFT_SPLIT_COUNT)
...
// Internal node access functions
...
void leaf_node_split_and_insert(Cursor *cursor, uint32_t key, Row *value)
{
    ...
    *leaf_node_next_leaf(old_node) = new_page_num;
    ...
    for (int32_t i = LEAF_NODE_MAX_CELLS - 1; i >= 0; i--)
    {
...
void create_new_root(Table *table, uint32_t right_child_page_num)
{
    ...
    // Initialize root as new internal node
...
void internal_node_insert(Table *table, uint32_t parent_page_num, uint32_t child_page_num)
{
    ...
    *internal_node_num_keys(parent) = original_num_keys + 1;
    ...
    if (child_max_key > get_node_max_key(table->pager, get_page(table->pager, right_child_page_num)))
    {
        *internal_node_child(parent, original_num_keys) = right_child_page_num;
        *internal_node_key(parent, original_num_keys) = get_node_max_key(table->pager, get_page(table->pager, right_child_page_num));
        *internal_node_right_child(parent) = child_page_num;
    }
    else
    {
...
void internal_node_split_and_insert(Table *table, uint32_t parent_page_num, uint32_t child_page_num)
{
    ...
    uint32_t child_max = get_node_max_key(table->pager, get_page(table->pager, child_page_num));
    ...
    *node_parent(get_page(table->pager, child_page_num)) = destination_page_num;
